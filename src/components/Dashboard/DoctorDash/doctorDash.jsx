...
function DoctorTable() {
  ...
  const handleEditClick = (doctor) => {
    ...
    setFormData({
      ...
      experience: safeArray(doctor.experience).map(e => ({
        description: e.description || "",
        date: e.date || ""
      })),
      fieldOfStudyId: doctor.fieldOfStudy?._id || "",
      universityId: doctor.university?._id || "",
      topicIds: safeArray(doctor.topic).map(t => t._id || t).filter(Boolean),
      profileFile: null,
    });
    setPreviewImage(doctor.profileImage?.fileUrl || "");
    setOpen(true);
  };
    ...
    try {
      ...
      const payload = {
        ...
        experience: safeArray(formData.experience)
          ...
          .map(e => ({ description: e.description.trim(), date: e.date || "" })),
      };

      let doctorRes;
      if (isEditing && editDoctorId) {
        doctorRes = await axios.put(`${BASE_URL}/api/doctors/${editDoctorId}`, payload, config);
      } else {
        doctorRes = await axios.post(`${BASE_URL}/api/doctors`, payload, config);
      }
...
// ### this code working 100% fine but we never use calendar for date yet
...
//   // Submit doctor and upload image via Media API
// // Submit doctor and upload profile image
// const handleSubmit = async () => {
//   try {
//     const token = localStorage.getItem("authToken");
//     if (!formData.doctorName?.trim()) {
