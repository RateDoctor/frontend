...
function AdminTable() {
  ...
  // -----------------------------
  // Fetch admins (pagination + search)
  // -----------------------------
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        await axios.post(`${BASE_URL}/api/admins/register`, payload, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleDelete = async (admin) => {
    ...
    setLoading(true);
    try {
      await axios.delete(`${BASE_URL}/api/users/${admin._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      <div className="dash-main">
        ...
        >
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            />
            {isEditing && <small>Leave blank to keep current password</small>}
          </div>
        </Modal>
